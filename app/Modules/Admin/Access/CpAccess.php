...
class CpAccess {
   ...
   public static function theName()
    ...
    public static function maskMobile($mobile)
    {
        ...
        $maskMobile = YC_Util::maskNewHp($mobile);
        $maskLog = [
            'mobile'      => $mobile,
            'create_uid'  => self::theUid(),
            'refer'       => $_SERVER['REQUEST_URI'],
        ];

        $maskId = MobileMaskLog::month(201812)->insertGetId($maskLog); 
        return ['mobile_mask' => $maskMobile, 'mobile_id' => $maskId];
     ...
     * 获取完整手机号
     */
    public static function decodeMaskMobile($maskId)
    {
        if (empty($maskId)) {
            return '参数错误';
        }
        $maskLog = MobileMaskLog::month(201812)->where('id', $maskId)->first();
        if (empty($maskLog)) {
            return '参数错误';
        }
        $maskLog->show_uid = self::theUid();
        $maskLog->save();
        return $maskLog->mobile;
    }
